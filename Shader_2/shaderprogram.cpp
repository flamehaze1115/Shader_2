...
bool ShaderProgram::compileShaderFromFile(const char *fileName, GLSLShader::GLSLShaderType type)
{
...
bool ShaderProgram::compileShaderFromString(const string& source, GLSLShader::GLSLShaderType type)
{
      if( handle <= 0 ) {
        handle = glCreateProgram();
        if( handle == 0) {
...
int ShaderProgram::getUniformBlockIndex(const char * name )
{
    return glGetUniformBlockIndex(handle, name);
}

bool ShaderProgram::fileExits(const string& fileName)
{
    struct stat info;
    int ret = -1;
